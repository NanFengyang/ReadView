...
//        pathCPaint.setStyle(Paint.Style.STROKE);
    ...
    public void computeScroll() {
        if (mScroller.computeScrollOffset()) {
            ...
            if(style.equals(STYLE_TOP_RIGHT)){
                setTouchPointF(x,y,STYLE_TOP_RIGHT);
            }else {
                setTouchPointF(x,y,STYLE_LOWER_RIGHT);
            }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()){
            ...
            case MotionEvent.ACTION_MOVE:
                setTouchPointF(event.getX(),event.getY(),style);
                break;
     ...
     * @param style
     */
    public void setTouchPointF(float x, float y, String style){
        ...
        switch (style){
            case STYLE_TOP_RIGHT:
                ...
                if(calcPointFCX(touchPointF,f)<0){//如果c点x坐标小于0则重新测量a点坐标
                    calcPointFAByTouchPointF();
                    calcPointFsXY(a,f);
                }
                postInvalidate();
                break;
     ...
     */
    private void calcPointFAByTouchPointF(){
...
//        canvas.drawPath(getPathC(),pathCPaint);
    ...
    private void drawPathCShadow(Canvas canvas){
        ...
        float minDisToControlPointF = Math.min(Math.abs(midpoint_ce - e.x), Math.abs(midpoint_jh - h.y));//中点到控制点的最小值
        ...
        if (style.equals(STYLE_TOP_RIGHT)) {
            ...
            right = (int) (c.x + minDisToControlPointF + deepOffset);
        } else {
            gradientDrawable = drawableCLowerRight;
            left = (int) (c.x - minDisToControlPointF - deepOffset);
     ...
     * @param f
     */
    private void calcPointFsXY(PointF a, PointF f){
        g.x = (a.x + f.x) / 2;
        g.y = (a.y + f.y) / 2;
        ...
        b = getIntersectionPointF(a,e,c,j);
        k = getIntersectionPointF(a,h,c,j);
     ...
     * @return 返回该点
     */
    private PointF getIntersectionPointF(PointF lineOne_My_pointOne, PointF lineOne_My_pointTwo, PointF lineTwo_My_pointOne, PointF lineTwo_My_pointTwo){
     ...
     * @return
     */
    private float calcPointFCX(PointF a, PointF f){
        PointF g,e;
        g = new PointF();
        e = new PointF();
        g.x = (a.x + f.x) / 2;
